...
Matrix Matrix::adjoint()
{
    ...
    int RDet = R.determinant();
    Matrix RAdjoint = RInverse * RDet;
    return RAdjoint;
}

Matrix Matrix::cofactor(int ridx, int cidx)
{
    Matrix R(*this), tmp;
    int cof_row, cof_col;
    if(R.rows_*R.cols_ == 4)
        cof_row = 1 - ridx;
        cof_col = 1 - cidx;
        tmp = Matrix();
        if (((ridx + cidx) % 2) == 0)
            tmp(0, 0) += R(cof_row, cof_col);
        else
            tmp(0, 0) += -1 * R(cof_row, cof_col);
        return tmp;
}
